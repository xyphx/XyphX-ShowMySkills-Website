...
export default function ProfileSetup() {
  ...
  const { user, updateUserProfile, checkUsernameAvailability, getUserProfile } =
    useAuth();
  const { uploadImage, uploadResume, isUploading, uploadProgress } =
    useFileUpload();
  ...
  const [formData, setFormData] = useState({
    ...
    showPhoneNumber: true, // Default to show phone number
  });
        ...
        try {
          ...
          if (userData) {
            ...
            setFormData({
              ...
              showPhoneNumber:
                userData.showPhoneNumber !== undefined
                  ? userData.showPhoneNumber
                  : true,
            });

            // Set image preview if user has a profile image
            // Do not use Google photo as preview
            ...
            // Do not use Google photo as initial preview
  ...
  const addSkill = (selectedSkill) => {
    if (selectedSkill && !formData.skills.includes(selectedSkill)) {
      setFormData((prev) => ({
        ...prev,
        skills: [
          ...prev.skills.filter((skill) => skill !== ""),
          selectedSkill,
          "",
        ],
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validExperience = formData.experience
        .filter(
          (item) =>
            item.title.trim() !== "" &&
      ...
      const validWorks = formData.works.filter(
      ...
      if (imageFile) {
        ...
        currentUserData.profileImage !== "/common-profile.png"
      ) {
      ...
      {formData[arrayName].map((item, index) => (
        ...
        >
          ...
          <textarea
            value={item.content}
            onChange={(e) =>
              handleArrayItemChange(arrayName, index, "content", e.target.value)
            }
          ...
          <input
            ...
            value={item.link}
            onChange={(e) =>
              handleArrayItemChange(arrayName, index, "link", e.target.value)
            }
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              value={item.title}
              onChange={(e) =>
                handleArrayItemChange("works", index, "title", e.target.value)
              }
          ...
          <div>
            ...
            <input
              ...
              value={item.link}
              onChange={(e) =>
                handleArrayItemChange("works", index, "link", e.target.value)
              }
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formData.type}
                    onChange={handleInputChange}
                    className={`w-full px-3 py-3 border rounded-xl text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 ${
                ...
                {formData.type && (
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleInputChange}
                      className={`w-full text-gray-700 px-3 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 ${
                ...
                {formData.type === "School" &&
                  (formData.course || formData.customCourse) && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Stream *
                      </label>
                      <select
                        ...
                        onChange={handleInputChange}
                        className={`w-full px-3 py-3 text-gray-700 border rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 ${
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">
                        {formData.isPublic
                          ? "Your profile is public and will be visible to everyone on the platform"
                          : "Your profile is private and will not be visible in search results"}
                      </p>
                    </div>
                    <div className="ml-6">
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 ${
                          formData.isPublic ? "bg-teal-600" : "bg-gray-300"
                        }`}
                        role="switch"
                  ...
                  <div className="space-y-3" ref={skillsRef}>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {formData.skills
                          ...
                          .map((skill, index) => (
                            <div
                              key={index}
                              className="flex items-center gap-2 px-3 py-1 bg-teal-100 border border-teal-300 text-teal-700 rounded-full text-sm"
                            >
                              <span>{skill}</span>
                              <button
                                type="button"
                                onClick={() =>
                                  removeSkill(formData.skills.indexOf(skill))
                                }
                    ...
                    {formData.skills.filter((skill) => skill.trim() !== "")
                      ...
                      <p className="text-sm text-gray-500">
                        Please select at least one skill from the dropdown
                        above.
                      </p>
                    )}
                ...
                {/* Toggle for show/hide phone number */}
                <div className="flex items-center mt-3">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 ${
                      formData.showPhoneNumber ? "bg-teal-600" : "bg-gray-300"
                    }`}
                    role="switch"
                    aria-checked={formData.showPhoneNumber}
                  >
                  ...
                  >
                    {formData.showPhoneNumber ? "Visible" : "Hidden"}
                  </span>
                </div>
                ...
                <div className="mb-4">
                  <button
                    type="button"
                    onClick={() => setShowWorksSection((prev) => !prev)}
                    className="px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors font-semibold"
                  >
                    {showWorksSection ? "Close Works Section" : "Add Work"}
                  </button>
                </div>
                {/* Works */}
                {showWorksSection && renderWorksSection()}

                {/* Experience Toggle Button */}
                <div className="mb-4">
                  <button
                    type="button"
                    onClick={() => setShowExperienceSection((prev) => !prev)}
                    className="px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors font-semibold"
                  >
                    {showExperienceSection ? "Close Experience Section" : "Add Experience"}
                  </button>
                </div>

                {/* Experience */}
                {showExperienceSection && renderExperienceSection()}
                  {/* Achievements Toggle Button */}
                <div className="mb-4">
                  <button
                    type="button"
                    onClick={() => setShowAchievementsSection((prev) => !prev)}
                    className="px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors font-semibold"
                  >
                    {showAchievementsSection ? "Close Achievements Section" : "Add Achievements"}
                  </button>
                </div>
                {/* Achievements */}
                {showAchievementsSection && renderAchievementsSection()}
